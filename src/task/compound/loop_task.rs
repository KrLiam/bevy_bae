//! Contains the [`Loop`] [`CompoundTask`]
...
/// After the last subtask is executed, execution unconditionally jumps back to the beginning.
#[derive(Debug, Component, Default, Reflect)]
#[reflect(Component)]
pub struct Loop;

impl CompoundTask for Loop {
    fn register_decompose(commands: &mut Commands) -> DecomposeId {
        commands.register_system(decompose_loop)
    }
}
...
fn decompose_loop(
    ...
    let initial_index = input.ctx_mut().plan.steps.len();
    ...
    input.ctx_mut().plan.steps.push(PlanStep::Jump {
        entity: input.compound_task,
        index: initial_index
    });
